...
export function MediaCard({ item }: MediaCardProps) {
  ...
  return (
    <Link href={`/media/${item.id}`} className="group flex gap-3 p-3 rounded-xl bg-card border border-card-border hover:border-primary/50 transition-all active:scale-[0.98]">
      <CoverImage 
        url={item.cover_url}
      ...
      <div className="flex flex-col flex-1 min-w-0 py-0.5">
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-auto mb-2">
          {item.my_status && item.my_status !== "Not Started" && (
            <Badge variant="secondary" className={`px-1.5 py-0 rounded text-[10px] font-medium border-none flex items-center ${getStatusColor(item.my_status)}`}>
