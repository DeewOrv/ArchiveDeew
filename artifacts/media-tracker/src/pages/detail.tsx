...
import { useGetMedia, getGetMediaQueryKey, useUpdateMediaProgress, useToggleMediaFavorite, useDeleteMedia, getListMediaQueryKey, getGetRecentMediaQueryKey, getGetContinueReadingQueryKey, getGetContinueWatchingQueryKey, getGetMediaStatsQueryKey } from "@workspace/api-client-react";
...
export default function Detail() {
  ...
  const progressMutation = useUpdateMediaProgress();
  const favoriteMutation = useToggleMediaFavorite();
  ...
  const handleProgress = async (delta: number) => {
    ...
    queryClient.setQueryData(getGetMediaQueryKey(id), { ...media, progress: newProgress });
    
    await progressMutation.mutateAsync({ id, data: { delta } });
    invalidateLists();
  };
  ...
  const handleFavorite = async () => {
    ...
    queryClient.setQueryData(getGetMediaQueryKey(id), { ...media, favorite: !media.favorite });
  ...
  return (
    <div className="flex flex-col pb-20 relative">
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2">
        <Button variant="secondary" size="icon" className="rounded-full bg-background/80 backdrop-blur" onClick={handleFavorite}>
        ...
        <Button variant="secondary" size="icon" className="rounded-full bg-background/80 backdrop-blur text-destructive" onClick={handleDelete}>
